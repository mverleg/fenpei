...
class DNNJob(NN_Job):
	...
	def __init__(self, name, weight, data, layers, batch_name = None):
		super(DNNJob, self).__init__(name = name, weight = weight, batch_name = batch_name, data = data, layers = layers)
		assert len(layers) == 2
		assert isinstance(data, ndarray)
		self.in_dimension = data.shape[-1] - 1

	def is_prepared(self):
		return super(DNNJob, self).is_prepared() and exists('%s/nn.exe' % self.directory)

	def is_complete(self):
		if not super(DNNJob, self).is_complete():
			return False
		try:
