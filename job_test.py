...
class TestJob(PyJob):
	...
	def __init__(self, name, weight, batch_name = None):
		super(TestJob, self).__init__(name = name, weight = weight, batch_name = batch_name)
	...
	def start(self, node, *args, **kwargs):
		super(TestJob, self).start(node, *args, **kwargs)
		cmds = [
			'cd %s' % self.directory,
