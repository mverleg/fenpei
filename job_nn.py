...
class NN_Job(Job):
    ...
    def __new__(cls, *args, **kwargs):
        cls.group_cls = NN_Job
        return super(NN_Job, cls).__new__(cls, *args, **kwargs)
    ...
    def __init__(self, name, weight, data, layers, batch_name = None, use_scaling = True, learning_rate = .7, train_error_function = ERRORFUNC_TANH, activation_function_hidden = SIGMOID_SYMMETRIC, activation_function_output = SIGMOID_SYMMETRIC):
        super(NN_Job, self).__init__(name = name, weight = weight, batch_name = batch_name)
        assert len(layers) >= 1
    ...
    def is_prepared(self):
        return super(NN_Job, self).is_prepared() and exists('%s/train.coord' % self.directory)
    
    def is_complete(self):
        return super(NN_Job, self).is_complete() and exists('%s/predictions.coord' % self.directory)
    
    def prepare(self, *args, **kwargs):
        if self.is_prepared():
            return False
        super(NN_Job, self).prepare()
