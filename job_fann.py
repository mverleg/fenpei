...
from fenpei.job_nn import NN_Job, NNJob
...
class FANNJob(NNJob):
	...
	def __init__(self, name, weight, data, layers, batch_name = None, use_scaling = True, learning_rate = .7, train_error_function = ERRORFUNC_TANH, activation_function_hidden = SIGMOID_SYMMETRIC, activation_function_output = SIGMOID_SYMMETRIC):
		super(FANNJob, self).__init__(name = name, weight = weight, batch_name = batch_name, data = data, layers = layers)
		assert len(layers) >= 1
	...
	def is_prepared(self):
		return super(FANNJob, self).is_prepared() and exists('%s/settings.in' % self.directory)

	def is_complete(self):
		return super(FANNJob, self).is_complete() and exists('%s/fit.fann' % self.directory) and exists('%s/min_range.scale' % self.directory)

	def prepare(self, *args, **kwargs):
		if self.is_prepared():
			return False
		super(FANNJob, self).prepare()
